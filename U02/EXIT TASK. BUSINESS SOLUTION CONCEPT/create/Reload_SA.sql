...
AS  
   PROCEDURE LOAD_DIM_BUYERS;
   PROCEDURE LOAD_DIM_SELLERS;

END pkg_ELT_DIM_PERSONS;



create or replace PACKAGE BODY                     U_DW_PERSONS.pkg_ELT_DIM_PERSONS
AS
   ...
   AS
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE U_DW_PERSONS.DIM_BUYERS';
    INSERT INTO U_DW_PERSONS.DIM_BUYERS
    WITH TMP_DIM AS 
    (SELECT 
        TU.ID_USER , 
        TU.F_NAME , 
        ...
        TU.PHONE , 
        TU.EMAIL , 
        TO_CHAR(TA.COUNTRY_ID) , 
        TO_CHAR(TA.CITY_ID),
        TO_CHAR(TA.STREET_ID),
        TU.IMAGE,
        TU.DATE_REG
        
    FROM 
        U_DW_PERSONS.T_USER TU
        LEFT JOIN 
        U_DW_REFERENCES.T_ADDRESSES TA
    ON
        TU.ID_USER = TA.ADRESS_ID)
    SELECT
    distinct
        TD.*
    FROM 
   ...
   AS
    ...
    EXECUTE IMMEDIATE 'TRUNCATE TABLE U_DW_PERSONS.DIM_SELLERS';
    INSERT INTO U_DW_PERSONS.DIM_SELLERS
    WITH TMP_DIM AS 
    (SELECT 
        TU.ID_USER , 
        TU.F_NAME , 
    ...
    FROM 
        ...
        U_DW_AUCTIONS.T_AUCTIONS_F AF
    ON 
     TD.ID_USER = AF.SELLER;
    commit;
   END LOAD_DIM_SELLERS;
