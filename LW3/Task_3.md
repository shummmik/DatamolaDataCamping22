...
```
Создав таблицу emp для нее создались сегменты таблицы и индекса.
```sql
...
--SYS_C007122          |INDEX       |
--SYS_C007142          |INDEX       |
--SYS_IOT_TOP_67046    |INDEX       |

```
После создания таблиц heap_addresses и iot_addresses появились сегменты: 
HEAP_ADDRESSES, SYS_C007142, SYS_IOT_TOP_67046. Из них таблица только одна. 
Это подтверждает тот факт, что данные IOT таблицы хранятся в индексе.
Проверим на плане как БД будет обращаться к данным.
```sql
...
|*  5 |    INDEX RANGE SCAN                  | SYS_C007142    |     4 |       |     1   (0)| 00:00:01 |
-------------------------------------------------------------------------------------------------------
...
|   2 |   TABLE ACCESS BY INDEX ROWID| EMP               |     1 |    54 |     2   (0)| 00:00:01 |
|*  3 |    INDEX UNIQUE SCAN         | EMP_PK            |     1 |       |     1   (0)| 00:00:01 |
|*  4 |   INDEX RANGE SCAN           | SYS_IOT_TOP_67046 |     4 |   184 |     1   (0)| 00:00:01 |
--------------------------------------------------------------------------------------------------
...
это осуществляется чтение из таблицы HEAP_ADDRESSES.

Вывод:
Наглядно увидели эффективность использования IOT таблиц. То, что нет необходимости в 
дополнительном чтении сегмента таблицы.
