CREATE OR REPLACE PACKAGE BODY u_dw_ext_references.pkg_load_ext_ref_languages
...
AS
   ...
   BEGIN
      ...
      EXECUTE IMMEDIATE 'TRUNCATE TABLE u_dw_ext_references.cls_languages_iso693';

      --Extract data
      INSERT INTO u_dw_ext_references.cls_languages_iso693 ( lng_3c_code
                                       , lng_2b_code
                                       , lng_2t_code
                                       , lng_1c_code
                                       , lng_scope
         ...
         SELECT lng_3c_code
              ...
              , lng_type
              , lng_desc
           FROM u_dw_ext_references.t_ext_languages_iso693;
   ...
   BEGIN
      ...
      DELETE FROM u_dw_references.w_lng_scopes lng
            WHERE lng.lng_scope_code NOT IN (     SELECT DISTINCT UPPER ( lng_scope ) FROM  u_dw_ext_references.cls_languages_iso693);

      --Merge Source data
      MERGE INTO u_dw_references.w_lng_scopes lng
           USING (  SELECT DISTINCT UPPER ( lng_scope ) AS lng_scope
                      FROM u_dw_ext_references.cls_languages_iso693
   ...
   BEGIN
      ...
      DELETE FROM u_dw_references.w_lng_types lng
            WHERE lng.lng_type_code NOT IN (     SELECT DISTINCT UPPER ( lng_type ) FROM  u_dw_ext_references.cls_languages_iso693);
   ...
   BEGIN
      ...
      DELETE FROM u_dw_references.w_languages lng
            WHERE lng.lng_3c_code NOT IN (     SELECT DISTINCT lng_3c_code FROM  u_dw_ext_references.cls_languages_iso693);
      ...
      MERGE INTO u_dw_references.w_languages lng
           USING (SELECT cls.lng_3c_code
                       ...
                       , cls.lng_desc
                    FROM u_dw_ext_references.cls_languages_iso693 cls
   ...
   BEGIN
      --truncate cleansing tables
      EXECUTE IMMEDIATE 'TRUNCATE TABLE u_dw_ext_references.CLS_LNG_MACRO2IND_ISO693';

      --Extract data
      INSERT INTO u_dw_ext_references.cls_lng_macro2ind_iso693 ( macro_lng_code
                                           , indiv_lng_code )
         SELECT macro_lng_code
              , indiv_lng_code
           FROM u_dw_ext_references.t_ext_lng_macro2ind_iso693;
      ...
      DELETE FROM u_dw_references.w_lng_links lnk
            WHERE ( lnk.parent_lng_id, lnk.child_lng_id, lnk.link_type_id ) NOT IN
                     (SELECT NVL ( m_lng.lng_id, -99 ) AS m_lng_id
                           ...
                           , 1 AS link_type_id
                        FROM u_dw_ext_references.cls_lng_macro2ind_iso693 cls
